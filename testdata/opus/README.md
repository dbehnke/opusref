...
The source is mono PCM. FFmpeg 9.0.1 and libopus 1.6.1 generated this set. The
general fixtures use the `voip` application, a 16 kbit/s rate, and one channel.
...
are three consecutive 20 ms packets from a deterministic 16 kHz synthetic
voice source. The fixture encoder uses 50 percent expected loss and a 24 kbit/s
rate to select in-band FEC. A 48 kHz Opus decoder produces 960 samples for each packet.
...
headers. It generates three consecutive FEC packets. It requires
`opus_packet_has_lbrr` to report LBRR in the recovery packet. It then simulates
loss of the prior packet and decodes 960 non-silent recovery samples with
`decode_fec=1`. The recovered samples must be closer to the omitted prior frame
than to the current frame. The script updates `SHA256SUMS`.
