...
func TestFloorRequestRetriesAndIgnoresMismatchedTransaction(t *testing.T) {
	...
	s := &udpSender{conn: clientConn, options: Options{OperationTimeout: 2 * time.Second}, session: 1, pending: map[uint64]chan wire.Packet{}, controlTokens: make(chan struct{}, 1)}
