...
const store = useSessionStore(); const recordings = ref<Recording[]>([]); const filter = ref('all'); const callsign = ref(''); const from = ref(''); const to = ref(''); const nextCursor = ref<string>(); const active = ref<string>(); const elapsed = ref(0); const loading = ref(true); const actionBusy = ref(false); const error = ref(''); const playbackError = ref(''); const capabilityError = ref(''); const retryRecording = ref<string>(); const adminPassword = ref(''); let audio: BrowserAudioSession | undefined
...
async function toggle(id: string) {
  ...
  if (audio && !audio.state.connected && !audio.state.listening) { await audio.close(); audio = undefined }
  ...
  retryRecording.value = id
  playbackError.value = ''
  try {
    const metadata = await api.request<Recording>(`/api/v1/recordings/${encodeURIComponent(id)}`)
    if (!audio) {
      audio = new BrowserAudioSession(store.csrf)
      audio.addEventListener('session-invalid', async () => { await store.refresh(); if (!store.authenticated) await router.replace('/login') })
      audio.addEventListener('state', () => { if (!audio) return; if (audio.state.error) playbackError.value = audio.state.error; if (audio.state.playback) { active.value = audio.state.playback.recordingId; elapsed.value = audio.state.playback.elapsedMs } else { active.value = undefined; elapsed.value = 0 } })
      if (!await audio.start()) {
        const permanentFailure = audio.state.capabilityError
        const failure = audio.state.error ?? 'Playback could not connect. Select Retry playback.'
        await audio.close(); audio = undefined
        if (permanentFailure) capabilityError.value = permanentFailure
        else playbackError.value = failure
        return
      }
    }
    if (audio.state.playback) audio.playback('playback_close', audio.state.playback.channelId)
    audio.openPlayback(metadata.id)
  } catch { playbackError.value = 'Playback could not start. Select Retry playback.'; await audio?.close(); audio = undefined }
...
<template>
  <div class="space-y-6"><header><h1 class="text-4xl font-black">Recordings</h1><p class="mt-2 text-slate-400">Play retained transmissions. Recordings expire after the configured retention period.</p></header>
    ...
    <p v-if="loading" role="status">Loading recordings…</p><p v-if="error" role="alert" class="text-red-200">{{ error }}</p><section v-if="playbackError && !capabilityError" class="surface border-amber-500 p-4" role="alert"><h2 class="font-bold text-amber-200">Playback did not connect</h2><p>{{ playbackError }}</p><button v-if="retryRecording" class="button-secondary mt-3" type="button" :disabled="actionBusy" @click="toggle(retryRecording)">Retry playback</button></section><section v-if="capabilityError" class="surface border-amber-500 p-4" role="alert"><h2 class="font-bold text-amber-200">Audio is not available</h2><p>{{ capabilityError }}</p><p class="mt-1 text-sm text-slate-400">You can continue to browse recording details and manage your account.</p></section>
    <ul v-if="recordings.length" class="space-y-4" aria-label="Recording library">
      <li v-for="item in recordings" :key="item.id" class="surface p-5"><div class="flex flex-wrap items-start justify-between gap-4"><div><div class="flex items-center gap-3"><h2 class="text-xl font-bold">{{ item.source_callsign }}</h2><StatusBadge :state="item.status" /></div><p class="mt-2 text-sm text-slate-400">{{ new Date(item.started_at).toLocaleString() }} · {{ Math.round(item.duration_ms / 1000) }} s · {{ item.end_reason }}</p></div><div class="flex gap-2"><button class="button-primary" type="button" :disabled="actionBusy || !!capabilityError" @click="toggle(item.id)">{{ active === item.id && audio?.state.playback?.state === 'playing' ? 'Pause' : 'Play' }}</button><button v-if="store.isAdmin" class="button-danger" type="button" :disabled="actionBusy || !adminPassword" @click="remove(item)">Delete</button></div></div>
        <div v-if="active === item.id" class="mt-5"><label class="label" :for="`seek-${item.id}`">Playback position: {{ Math.round(elapsed / 1000) }} seconds</label><input :id="`seek-${item.id}`" v-model.number="elapsed" class="seek-control w-full accent-cyan-300" type="range" min="0" :max="item.duration_ms" step="20" @change="seek"></div></li>
