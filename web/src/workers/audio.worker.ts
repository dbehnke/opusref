...
function configureDecoder() {
  decoder = new AudioDecoder({
    ...
    output: (audio: any) => {
      ...
      scope.postMessage({ type: 'pcm', pcm, timestamp: audio.timestamp }, [pcm.buffer])
    },
  })
...
scope.onmessage = event => {
  ...
  if (data.type === 'pcm') {
    ...
    encoder.encode(new AudioData({ format: 'f32-planar', sampleRate: 48000, numberOfFrames: 960, numberOfChannels: 1, timestamp: timestamp * 1000000 / 48000, data: data.pcm }))
    return
  }
  if (data.type === 'media') {
    ...
    if (decoder.decodeQueueSize >= 25) return fail('receive_overload', 'Audio restarted because the receive queue was full.')
