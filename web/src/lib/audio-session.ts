...
export class BrowserAudioSession extends EventTarget {
  ...
  private readonly playbackRequests = new Map<string, 'open' | 'pause' | 'resume' | 'seek' | 'close'>()
  ...
  private onSocket(event: SocketEvent) {
    ...
    else if (control.type === 'playback_opened') { const body = control.body as { channel_id: string; recording_id: string; duration_ms: number; status: 'complete' | 'partial' }; try { parseChannelId(body.channel_id); if (control.request_id) this.playbackRequests.delete(control.request_id); this.resetPlayout(); this.playbackExpectedSequence = 0; this.playbackAccepting = true; this.update({ playback: { channelId: body.channel_id, recordingId: body.recording_id, durationMs: body.duration_ms, status: body.status, state: 'playing', elapsedMs: 0 } }); this.startPlaybackClock() } catch { this.socket.close() } }
    else if (control.type === 'playback_state') { const body = control.body as { channel_id: string; state: 'playing' | 'paused' | 'closed'; elapsed_ms: number }; const action = control.request_id ? this.playbackRequests.get(control.request_id) : undefined; if (control.request_id) this.playbackRequests.delete(control.request_id); if (this.state.playback?.channelId === body.channel_id) { if (body.state === 'closed') this.closePlayback(); else { this.resetPlayout(); if (action === 'seek') this.playbackExpectedSequence = 0; this.playbackAccepting = body.state === 'playing'; this.update({ playback: { ...this.state.playback, state: body.state, elapsedMs: body.elapsed_ms } }); if (body.state === 'playing') this.startPlaybackClock(); else clearInterval(this.playbackTimer) } } }
  ...
  private sendPlayback(type: string, action: 'open' | 'pause' | 'resume' | 'seek' | 'close', body: unknown) { const requestID = this.send(type, body); if (requestID) this.playbackRequests.set(requestID, action) }
